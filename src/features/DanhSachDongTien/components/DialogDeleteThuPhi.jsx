...
import {fetchDeleteKhoanthu, fetchDeleteThuphi} from "src/features/DanhSachDongTien/khoanthuSlice";


const DialogDeleteThuPhi = NiceModal.create(({id, onAlert}) => {
  ...
  const handleDelete = () => {
    ...
    dispatch(fetchDeleteThuphi(data))
  }
  useEffect(() => {
    if (data.status === "delete thuphi") {
      modal.hide()
      onAlert()
    }
  }, [data])
  return (
    <Dialog open={modal.visible} onClose={() => modal.hide()}>
      ...
      <Stack height={80} width={400} p={2} direction='row' alignItems="center" spacing={1}>
        ...
        <Typography fontWeight={410}>Bạn có chắc chắn muốn xóa thu phí ?</Typography>
      </Stack>
      {/*<Divider sx={{backgroundColor:colors.grey[500]}}/>*/}
...
export default DialogDeleteThuPhi
