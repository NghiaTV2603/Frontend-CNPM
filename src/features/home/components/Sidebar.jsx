...
import {Button, colors, Stack, Avatar, Typography, Divider} from "@mui/material";
...
export default function Sidebar(props) {
  ...
  const user = {
    name: 'Nghia Tran Van',
    avatar: 'https://vcdn1-thethao.vnecdn.net/2022/12/10/messi-3-jpeg-1670629980-167062-7567-4192-1670631569.jpg?w=680&h=0&q=100&dpr=1&fit=crop&s=KhxAvzMM7Em5ke2KWF77wA',
    level: 'admin'
  }
    ...
    <Stack>
      <Stack height="100%" color={colors.grey[200]}>
        <Stack className='tag-name' p={2}>
          <Stack direction='row' alignItems='center' p={2} bgcolor={colors.grey[800]} borderRadius={4} height={80}>
            <Avatar sx={{height: 48, width: 48}} src={user.avatar}/>
            <Stack ml={1.5}>
              <Typography fontSize={16} color={colors.grey[100]}>{user.name}</Typography>
        ...
        <Stack py={2}>
          {
            contentsSidebar.map((content, index) => (
              ...
              >
                ...
                <Typography color={numTab === index ? colors.grey[100] : ''} px={2} fontSize={17}>
                  {content.title}
                </Typography>
              </Stack>
            ))
